...
const getExpertiseStyles = (index: number) => {
    const styles = [
        { iconBg: "bg-gradient-to-br from-blue-400 to-indigo-600 shadow-blue-500/30" },
        { iconBg: "bg-gradient-to-br from-teal-300 to-emerald-500 shadow-teal-500/30" },
        { iconBg: "bg-gradient-to-br from-purple-400 to-pink-500 shadow-purple-500/30" },
                    ...
                    {expertise.length > 0 ? (
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {expertise.map((item, index) => {
                                const style = getExpertiseStyles(index);
                                return (
                                    <Link key={index} href="/services" className="block relative overflow-hidden bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/15 hover:border-white/35 hover:bg-white/15 hover:shadow-2xl hover:shadow-black/20 hover:-translate-y-1.5 transition-all duration-300 group">
                                        ...
                                        <div className="flex items-center gap-2 text-sm font-semibold text-white/90 group-hover:text-white transition-colors">
                                            <span className="relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-white group-hover:after:w-full after:transition-all after:duration-300">Discover Services</span>
                    ...
                    {affiliations.length > 0 ? (
                        <div className="grid md:grid-cols-2 gap-8">
                            {affiliations.map((item, index) => (
                                <div key={index} className="relative overflow-hidden bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group">
                                    {/* Top Color Accent Line */}
                                    <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${item.gradient}`}></div>

                                    <div className="flex gap-5 sm:gap-6 items-start">
                                        {/* Glowing Medal/Award Badge */}
                                        <div className={`shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br ${item.gradient} flex items-center justify-center text-white shadow-md shadow-blue-500/10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                                                <span className="text-xs text-gray-500 font-semibold flex items-center gap-1">
                                                    ...
                                                    {item.description.replace("Awarded on ", "")}
                                                </span>
                                            </div>
                                            <h3 className="text-lg sm:text-xl font-extrabold text-gray-900 mb-1 leading-snug group-hover:text-[#0066ff] transition-colors truncate-2-lines">{item.org}</h3>
                                            <span className={`inline-block px-3 py-1 rounded-full bg-linear-to-r ${item.gradient} text-white text-[11px] font-bold tracking-wide uppercase`}>
                                                {item.role}
                                            </span>
                                        </div>
                                    </div>

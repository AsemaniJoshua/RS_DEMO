        ...
        <div className="p-8">
            ...
            <div className="block md:hidden space-y-4">
                {blogs.length === 0 ? (
                    ...
                    blogs.map((blog) => (
                        <div key={blog.id} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm space-y-3">
                            <div className="flex items-start gap-3">
                                <div className="h-16 w-24 relative rounded-md overflow-hidden bg-gray-100 flex-shrink-0">
                                    {blog.featured_image ? (
                                        <img 
                                ...
                                <div className="min-w-0 flex-1">
                                    <Link href={`/admin/blog/${blog.id}`} className="font-semibold text-gray-900 hover:text-[#0066ff] transition-colors line-clamp-2">
                                        {blog.title}
                                    </Link>
                                    <div className="text-xs text-gray-500 mt-1">
                                        By {blog.author ? `${blog.author.first_name} ${blog.author.last_name}` : 'Unknown'}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center text-sm border-t border-gray-100 pt-2">
                                <div className="text-gray-500">
                                    Published: <span className="font-medium text-gray-900">{blog.published_at ? new Date(blog.published_at).toLocaleDateString() : '-'}</span>
                                </div>
                                <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadge(blog.status)}`}>
                                    {blog.status}
                                </span>
                            </div>

                            <div className="flex justify-end gap-1 items-center border-t border-gray-100 pt-2" onClick={(e) => e.stopPropagation()}>
                                <button

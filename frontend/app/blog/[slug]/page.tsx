...
export default function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    useEffect(() => {
        const fetchBlog = async () => {
            try {
                setLoading(true);
                // Try fetching by slug first (slug-based URLs)
                const response = await publicService.getPublicBlogBySlug(slug);
                ...
                // If slug lookup fails (404), try fetching by ID for backwards compatibility
                try {
                    const fallbackResponse = await publicService.getPublicBlogById(slug);
                    const blogData = (fallbackResponse.data as any)?.blog || fallbackResponse.data;
                    if (blogData) {
                        // Redirect to the canonical slug URL if we got the blog by ID
    ...
    const getShareUrl = () => {
        if (!blog) return '';
        // Use slug for shareable URL (canonical)
    ...
    const handleShare = (platform: 'twitter' | 'linkedin' | 'whatsapp' | 'copy') => {
        ...
        const cleanExcerpt = blog.excerpt?.replace(/<[^>]*>/g, '').slice(0, 200) || '';
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pt-6 border-t border-gray-100">
                        ...
                        <div className="flex items-center gap-2 bg-gray-50 hover:bg-gray-100/70 p-1.5 rounded-full border border-gray-100 transition-colors self-start sm:self-auto">
                            ...
                            <button 
                                ...
                                className="w-8 h-8 rounded-full bg-white hover:bg-gray-800 hover:text-white text-gray-600 shadow-xs hover:shadow-md transition-all duration-300 flex items-center justify-center cursor-pointer"

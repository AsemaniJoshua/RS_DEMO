...
export default function DashboardSpeakingDetailClient() {
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                const data = await speakingService.getSpeakingEventById(id);
                ...
                <div className="lg:col-span-2 space-y-6 min-w-0">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-8">
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                            <div>
                                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">{event.title}</h1>
                                <p className="text-gray-500 text-base sm:text-lg">{event.venue}</p>
                            </div>
                            {event.status && (
                                <span className={`px-4 py-1.5 rounded-full text-sm font-semibold border self-start ${getStatusColor(event.status)}`}>
                                    {event.status}
                                </span>
                            )}
                        </div>
